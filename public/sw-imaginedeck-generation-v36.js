(() => {
  ...
  const IMAGINEDECK_GENERATION_VERSION = 36;
  ...
  let stableGenerationRefreshPromise = null;
  ...
  async function readGenerationEntries(cacheName) {
    if (typeof cacheName !== 'string' || !cacheName.startsWith(GENERATION_CACHE_PREFIX)) {
      return null;
    }
  ...
  async function writeActiveGenerationPointer(cacheName, signature) {
    ...
    // A single Cache.put is the publication point. Competing service-worker
    // globals can only replace this complete pointer; they cannot interleave
    // the immutable generation contents behind it.
  ...
  async function mirrorGenerationForLegacyWatchdog(entries) {
    // Compatibility mirror for the current window watchdog. Delivery and
    // navigation pinning never read this mutable cache in v36; authoritative
    // reads use the immutable generation selected by the atomic pointer.
  ...
  async function cleanupOldGenerationCaches(activeCacheName) {
    ...
    await Promise.all(
      cacheNames.map(async cacheName => {
        ...
        if (Number.isFinite(createdAt) && now - createdAt > GENERATION_RETENTION_MS) {
  ...
  async function publishImmutableGeneration(entries, signature) {
    ...
    try {
      ...
      try {
        ...
        // The immutable generation and pointer are already authoritative. A
        // compatibility-mirror failure may delay watchdog acknowledgement, but
        // must not invalidate or delete the published generation.
        console.warn(
  ...
  async function readServerGenerationSignature() {
    const signatures = [];
    // Preserve a canonical order and avoid certifying a set based only on the
    // completion order of parallel requests.
    for (const pathname of ATOMIC_IMAGINEDECK_ASSET_PATHS) {
      signatures.push(await serverAssetSignature(pathname));
    }
  ...
  async function refreshStableGenerationFromNetwork() {
    if (stableGenerationRefreshPromise) {
      return stableGenerationRefreshPromise;
    }

    stableGenerationRefreshPromise = (async () => {
      ...
      for (let attempt = 1; attempt <= MAX_STABLE_FETCH_ATTEMPTS; attempt += 1) {
        ...
        console.warn(
          '[ServiceWorker] ImagineDeck release changed while staging; retrying the complete generation.',
          lastMismatch
        );
      }
      ...
      stableGenerationRefreshPromise = null;
    });
  ...
  async function handleGenerationDocumentNavigation(request, evt) {
    ...
    try {
      // app.html is the generation boundary. Online navigations must observe a
      // stable complete release before the immutable pointer is published.
      await refreshStableGenerationFromNetwork();
    } catch (error) {
      console.warn(
        '[ServiceWorker] Failed to refresh a stable ImagineDeck release before navigation; retaining the current immutable generation.',
        error
      );
    }

    const generation = await ensureActiveGeneration().catch(() => null);
    if (!generation) {
    ...
    try {
      const pinnedGeneration = await pinActiveGenerationForClient(
        navigationClientId
      );
  ...
  // Replace the mutable active-cache refresh with stable immutable generation
  // publication. Existing callers, including the watchdog promotion probe, now
  // receive the same cross-worker-safe behavior.
  refreshAtomicAssetSetFromNetwork = refreshStableGenerationFromNetwork;
  pinActiveImagineDeckGeneration = pinActiveGenerationForClient;

  handleNetworkFirstAssetRequest = function handleNetworkFirstAssetRequestV36(
    request,
    evt
  ) {
    const requestUrl = new URL(request.url);
